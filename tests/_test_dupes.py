...
def test_dupes(gui, pose_project_root):
    ...
    dw = meta.data_widget
    before = (_count(dw.individual_layout, "Individual:"), _count(dw.coords_groupbox_layout, "Keypoint:"),
              _count(dw.coords_groupbox_layout, "Space:"), len(dw.individual_groupbox.findChildren(QComboBox)))
    ...
    QApplication.processEvents()
    after = (_count(dw.individual_layout, "Individual:"), _count(dw.coords_groupbox_layout, "Keypoint:"),
             _count(dw.coords_groupbox_layout, "Space:"), len(dw.individual_groupbox.findChildren(QComboBox)))
    ...
    print("VISIBLE indiv combos", [c.objectName() for c in dw.individual_groupbox.findChildren(QComboBox) if not c.isHidden()])
    assert before == after
