"""Settings ▸ Open project settings folder… follows ``app_state.project_path``: live
while a project folder is set and exists, greyed out otherwise."""
...
def _settings_action(shell, text):
    menu = next(a.menu() for a in shell.menuBar().actions() if a.text() == "&Settings")
...
def test_greyed_out_without_a_project_and_live_once_one_is_set(qtbot, tmp_path):
    ...
    action = _settings_action(shell, "Open project settings folder…")
...
def test_opens_the_project_folder(qtbot, tmp_path, monkeypatch):
    ...
    _settings_action(shell, "Open project settings folder…").trigger()
