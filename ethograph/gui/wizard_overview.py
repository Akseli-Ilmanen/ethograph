...
def open_with_chooser(path: Path) -> None:
    """Open *path* in a program the user picks.

    Windows shows its "How do you want to open this file?" chooser, where the
    user can also make the choice permanent. macOS and Linux have no such
    prompt to call, so the file goes to the registered default handler, and the
    OS itself asks when there is none.
    """
    try:
        ...
        notify_dialog(f"Could not open the notebook:\n{path}\n{exc}", "warning", "Open notebook")
...
class NCWizardDialog(QDialog):
    def __init__(self, app_state, io_widget, parent: QWidget | None = None):
        ...
        self._apply_project_defaults(self._state)
    ...
    def _populate_trials_from_sources(self) -> str | None:
        ...
        state = self._state
        try:
            table = discover_media(_source_specs(state))
    ...
    def _finish(self) -> None:
        """Write the notebook and close. The wizard is a notebook generator: it never
        pairs, writes an alignment or a session file itself; the notebook does that,
        so it stays the one record of how the session was built."""
        ...
        if state.mode == "pair":
            target = "the session folder it names"
        else:
            target = "the folder holding the session.nwb it writes"
        notify_dialog(
            f"Notebook written:\n{notebook}\n\nRun it, then open {target} on the start page.",
            "info",
            "Notebook written",
            self,
        )
        if state.open_notebook:
            open_with_chooser(notebook)
        self.accept()
...
def _rig_spec(state: WizardState):
    ...
    for stream, cfg in (("video", state.video), ("pose", state.pose), ("audio", state.audio)):
        ...
        if session_wide:
            folder = str(cfg.files[0] if cfg.files else cfg.single_file_path)
        else:
            folder = str(Path(cfg.folder_path).resolve())
        sources.append(
            RigSource(
                stream=stream,
                folder=folder,
                ...
                software=cfg.source_software if stream == "pose" else None,
                extension=None if session_wide else cfg.extension,
    ...
    return RigSpec(
        ...
        individuals=list(state.individuals),
    )
