...
worth showing it more often next time. `Ctrl+T` (or the **Hard trial** box in
the section) flags the current trial as `hard` in the metadata table's
`difficulty` column, and again puts it back to `normal`. The column is a
checklist in the trials table's filter like `curated`, so "show me the hard
trials" is one click, and a training run reads it back: in the segmentation
pipeline, `train.oversample: {column: difficulty, weights: {hard: 3.0}}` draws a
hard trial three times as often ({doc}`segment/config`).
