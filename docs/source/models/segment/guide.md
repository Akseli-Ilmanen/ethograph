(target-segment-guide)=
# Full guide

Every stage of the segmentation pipeline with the choices put back in: how a
workflow is split into a search and a cross-validation, what a sample is, how
the materialised dataset is laid out, which architectures exist and how to
compare them, and how predictions come back to the GUI. {doc}`quickstart` is
this page with every choice made for you; {doc}`config` lists every key.
...
confidence, and every label you confirm becomes `curated`.
