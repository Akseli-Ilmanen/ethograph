...
ethograph/spot/               # Pixel point-event spotting (E2E-Spot); docs: docs/source/models/spot/
    ...
    features.py               # The pose side: the listed features per trial (features/*.npz + features.json), the z-scored block (features/block/) the pixel model reads
...
- **Video features from files attach in memory, matched by video name** (`io/video_feature_files.py`: a folder or `.npy` files dropped on the window, or a session's `video_feature_folders` in a segment config; one variable per folder named after it, listed with its path under the popup's Video features; sampled onto the trial clock through the alignment; a video without a file reads NaN; never written back by `TrialTree.save`). Covered by `tests/test_unit/test_video_feature_files.py` + `test_source_popup_video_features.py`.
